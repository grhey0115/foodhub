...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //document.getElementById('submit-summary').addEventListener('click', submitOrder);
    ...
    document.getElementById('summary-list').addEventListener('click', function(event) {
        if (event.target.classList.contains('delete-item')) {
            const summaryItem = event.target.parentNode;
            summaryItem.remove();
            updateSummaryTotal(); // Recalculate total after deletion
        }
    });
});
...
function paySummary() {
    ...
    if (isNaN(payment) || payment < totalPrice) {
        ...
        const transaction = {
            ubtransactId: generateUniqueID(),
            orderDate: new Date().toISOString(),
            description: buildDescriptionFromSummary(),
            size: 'Mixed',  // You can update this based on actual size logic
...
function storeTransaction(transaction) {
    ...
    // Optionally, refresh the transactions table if on the same page
